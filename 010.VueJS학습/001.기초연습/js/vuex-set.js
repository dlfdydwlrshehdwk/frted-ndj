...
/************************************** 
    ...
    4. 버전 : Vue 2.x 버전일경우 Vue Router 3.x 버전을 사용함
...
new Vue({
    ...
    // 제이쿼리는 DOM에 직접 작용하므로 mounted에 구현한다.
    mounted : function(){
        // 링크 클릭시 a에 클래스 on주기
        ...
        function showBx(){
            // 이미지와 설명박스 순서대로 나타나기
            $('main img').css({opacity:0}).stop().delay(500).fadeTo(1000,1)
            $('main p').css({opacity:0}).stop().delay(1000).fadeTo(1000,1)
        }
    },
})