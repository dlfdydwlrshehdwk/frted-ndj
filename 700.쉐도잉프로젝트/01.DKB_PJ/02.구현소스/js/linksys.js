...
window.addEventListener("DOMContentLoaded",() => {
    ...
    for(let x of link){ //x는 각 a요소
        ...
        x.onclick = () => {
            ...
            // 만약 이미지가 있으면 img요소의 alt 읽어서 
            // atxt변수에 재할당하기
            // if(true)
            // if(null)
            // if(undefined)
            // ->> 데이터가 null인 경우는 if문에서 false와 같이 취급
            // ->> 데이터가 undefined인 경우는 if문에서 false와 같이 취급
            // 만약 요소가 없으면 null 이 리턴된다.
            // undefined는 변수의 값이나 함수가 생성되지 않은 경우 리턴되는 기본할당값이다.
