...
// 이퀄오른쪽에 괄호() 를 하면 함수가 바로 실행되므로 하지말것.
...
/*********************************************** 
    ...
    document.getElementById("아이디명").onclick = 함수;
    fucntion 함수(){
        var tg = document.querySelector(".클래스명");

        tg.classList.toggle("클래스명");
    }
    -> 요소에 클래스를 넣거나 뺴는 등 필요작업을 
    하는 객체

    ((관련메서드))
    1. add(클래스명) : 클래스추가
    2. remove(클래스명) : 클래스 제거
    3. toggle(클래스명) : 클래스 추가/제거 자동
    4. contains(클래스명) : 클래스있으면 true 없으면 false
