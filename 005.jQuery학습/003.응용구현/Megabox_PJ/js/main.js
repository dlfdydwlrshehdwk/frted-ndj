...
$(function () { //// jQB2 //////////////////////////
    ...
    $('.btnpp img').click(function(){
        // 동영상 멈춤상태 알아내기 - paused 속성!으로 알아냄
        // 결과 : true - 멈춤 , false - 재생중(멈춤아님)
        let paused_sts = mv.get(0).paused;
        // console.log('비디오가멈췃니?',paused_sts)

        // 1. 멈춤아니면(false) 동영상 멈춰!
    ...
    mv.on('timeupdate',function(){
        ...
        // 1. 비디오 현재시간 가져오기 : currentTime속성 (비디오현재간간)
        ...
        // 현재시간이 0일 경우저전체시간이 안나옴(if문으로 막기)
        // 현재시간이 숫자가 아니라면
        if(!isNaN(ftime)){
            let percent = ctime / ftime * 100
            console.log('진행율',percent.toFixed(1))
            // 소수점변수 .toFixed(자릿수) -> 특정자리수까지만 소수점표시
